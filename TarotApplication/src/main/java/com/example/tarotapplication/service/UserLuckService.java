...
public class UserLuckService {
    ...
    public UserLuck addLuck(UserLuck userLuck) {
        ...
        if (userLuckRepository.existsByUser(user)) {
            throw new RuntimeException("Luck database already created.");
        }
        userLuck.setUser(user);
        return userLuckRepository.save(userLuck);
    }
    
    public void changeLuckWithCurrentUser(Integer luck){
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        User user = userRepository.findByUsername(username);
        userLuckRepository.findByUser(user).modifyLuck(luck);
        return;
    }
